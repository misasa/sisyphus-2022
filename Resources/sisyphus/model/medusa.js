    ...
    model.medusa.getResourceURLwithAuth = function(_record, _args) {
        var _path = model.medusa.getResourcePath(_record);
    ...
    model.medusa.getSurfaceURLwithAuth = function(_record, _args) {
        //var _path = model.medusa.getResourcePath(_record);
    ...
    model.medusa.client = function(_args) {
    ...
    model.medusa.get = function(_args) {
        var client = model.medusa.client(_args);
    ...
    model.medusa.getClassifications = function(_args) {
        model.medusa.get({
    ...
    model.medusa.getWithAuth = function(_args) {
        model.medusa.sendWithAuth(_args, 'GET');
    };

    model.medusa.putWithAuth = function(_args) {
        model.medusa.sendWithAuth(_args, 'PUT');
    };

    model.medusa.postWithAuth = function(_args) {
        model.medusa.sendWithAuth(_args, 'POST');
    };

    model.medusa.deleteWithAuth = function(_args) {
        Ti.API.info("deleting...");
        model.medusa.sendWithAuth(_args, 'DELETE');
    };

    model.medusa.sendWithAuth = function(_args, method) {
        var client = model.medusa.client(_args);
        var url = Ti.App.Properties.getString('server') + _args.path;
        Ti.API.info(url);
        ...
        //if(!si.nfc.isEnabled()) {
        if( Ti.App.Properties.getBool('NfcEnabled') ) {
    ...
    model.medusa.getToken = function(_args) {
        model.medusa.postWithAuth({
            path : PATH_TOKEN,
    ...
    model.medusa.getAccountInfo = function(_args) {
        model.medusa.getWithAuth({
            //path : PATH_STONE + '/1' + PATH_JSON,
    ...
    model.medusa.getRecordFromGlobalId = function(_args) {
        if (!_args.global_id){
            _args.onerror();
            return
        }
        ...
        model.medusa.getWithAuth({
            ...
            onsuccess : function(response) {
                ...
                model.medusa.getWithAuth({
                    args : _args.args,
                    path : model.medusa.getClassPath(record) + '/' + record.id + PATH_JSON,
                    username : _args.username,
                    password : _args.password,
                    onerror : _args.onerror,
                    onsuccess : function(_response) {
                        ...
                        model.medusa.getWithAuth({
                            args : _args.args,
                            path : model.medusa.getClassPath(record) + '/' + record.id + PATH_ATTACHMENT_FILE_JSON,
                            username : _args.username,
                            password : _args.password,
                            onerror : _args.onerror,
                            onsuccess : function(__response) {
                                ...
                                if (__response.length > 0) {
                                    var attachmentFile = __response[0];
    ...
    model.medusa.delete = function(_args) {
        var mr = _args.global_id.match(/.+=(.+)/);
        if (mr != null) {
            _args.global_id = mr[1];
        }
        model.medusa.deleteWithAuth({
    ...
    model.medusa.getResourcePath = function(_record) {
        var class_path = model.medusa.getClassPath(_record);
        //var path = [class_path, _record.id + JSON_PATH].join('/')
        var path = class_path + '/' + _record.id + PATH_JSON;
        return path;
    };

    model.medusa.className2Path = function(_className) {
        var path = '';
        switch(_className) {
            case CLASSNAME_STONE:
                path = PATH_STONE;
    ...
    model.medusa.getResourcePropertyPath = function(_record) {
        var class_path = model.medusa.getClassPath(_record);
        //var path = [class_path, _record.id + JSON_PATH].join('/')
        return class_path + '/' + _record.id + PATH_RECORED_PROPERTY_JSON;
    };

    model.medusa.getCollectionPath = function(_className) {
        var class_path = model.medusa.className2Path(_className);
        return class_path + PATH_JSON;
    };

    model.medusa.getClassPath = function(_record) {
        var path = '';
        ...
        var path = model.medusa.className2Path(_record._className);
        return path;
    };

    model.medusa.getLinkPath = function(_parent, _child) {
        var path = '';
        path += model.medusa.getClassPath(_parent);
        path += '/' + _parent.id;

        if ((_parent._className == CLASSNAME_STONE) && (_child._className == CLASSNAME_STONE)) {
            path += PATH_DAUGHTERS;
        } else {
            path += model.medusa.getClassPath(_child);
        }
        path += '/' + _child.id;
        path += PATH_JSON;
        return path;
    ...
    model.medusa.inventoryPath = function(_parent, _child) {
        var path = '';
    ...
    model.medusa.createLink = function(_parent, _child, _args) {
        if (((_parent._className == CLASSNAME_BOX) && (_child._className == CLASSNAME_STONE)) || ((_parent._className == CLASSNAME_BOX) && (_child._className == CLASSNAME_BOX))) {
            model.medusa.postWithAuth({
                args : _args.args,
                path : model.medusa.inventoryPath(_parent, _child),
                username : _args.username,
                password : _args.password,
                onsuccess : function(response) {
                    _args.onsuccess(response);
            ...
            model.medusa.putWithAuth({
                args : _args.args,
                path : model.medusa.getLinkPath(_parent, _child),
                username : _args.username,
                password : _args.password,
                onsuccess : function(response) {
                    _args.onsuccess(response);
    ...
    model.medusa.getImageUploadPath = function(_parent) {
        var path = '';
...
//                break;
    ...
    model.medusa.uploadImage = function(_args) {
        var params = {};
        params['attachment_file[data]'] = _args.data;
        if (_args.record.group_id){
            params['attachment_file[record_property_attributes][user_id]'] = _args.record.user_id;
            params['attachment_file[record_property_attributes][group_id]'] = _args.record.group_id;
        }
        model.medusa.postWithAuth({
            args : params,
            path : model.medusa.getImageUploadPath(_args.record),
            username : _args.username,
            password : _args.password,
            onsuccess : function(response) {
                _args.onsuccess(response);
    ...
    model.medusa.update_attributes = function(_record, _args) {
        var params = {};
        ...
        model.medusa.putWithAuth({
            args : params,
            path : model.medusa.getResourcePath(_record),
            username : _args.username,
            password : _args.password,
            onsuccess : function(response) {
                model.medusa.reload(_record,{
                    username: _args.username,
                    password: _args.password,
                    onerror : _args.onerror,
    ...
    model.medusa.reload = function(_record, _args) {
        var path = model.medusa.getResourcePath(_record);
        ...
        model.medusa.getWithAuth({
            ...
            onsuccess : function(_response) {
                ...
                model.medusa.load_property(_response, _args);
            }
        });

    };

    model.medusa.load_property = function(_record, _args) {
        var path = model.medusa.getResourcePropertyPath(_record);
        model.medusa.getWithAuth({
            path : path,
            username : _args.username,
            password : _args.password,
    ...
    model.medusa.create = function(_className, _args) {
        var params = {};
        var properties = null;
        var _paramName = _className.toLowerCase();
        for (var key in _args.args){
        ...
        var path = model.medusa.getCollectionPath(_className);
        model.medusa.postWithAuth({
            args : params,
            path : path,
            username : _args.username,
            ...
            onsuccess : function(_stone) {
                if (_stone) {
                    ...
                    model.medusa.load_property(record, {
                        username : _args.username,
                        password : _args.password,
                        onsuccess : function(_record) {
                            if (properties){
                                // update record_properties
                                model.medusa.update_attributes(_record, {
                                    args: properties,
                                    username : _args.username,
                                    password : _args.password,
    ...
    model.medusa.createNewStone = function(_args) {
        model.medusa.create('Specimen', _args);
    };

    model.medusa.createNewBox = function(_args) {
        model.medusa.create('Box', _args);
    };

    model.medusa.update = function(_className, _args) {
        var params = {};
        var properties = null;
        var _paramName = _className.toLowerCase();
        for (var key in _args.args){
        ...
        var path = model.medusa.getResourcePath(_args.args);
    ...
    model.medusa.getRecords = function(_args) {
        var params = "?";
        var queries = [];
